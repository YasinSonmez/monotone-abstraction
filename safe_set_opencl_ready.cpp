...
/**
 * OpenCL-Ready Safe Set Computation for Vehicle Following
 * Uses C-style arrays instead of STL containers for GPU compatibility
 * 
 * Usage: ./safe_set_opencl_ready [2d|3d] [precomp|noprecomp] [seq|parallel]
 * Example: ./safe_set_opencl_ready 3d precomp seq
 */
...
// Maximum sizes for pre-allocation (OpenCL requires fixed sizes)
const int MAX_STATE_DIM = 3;
const int MAX_INPUT_DIM = 1;
const int MAX_DISTURBANCE_DIM = 1;
const int MAX_BASIS_ELEMENTS = 2000;  // Pre-allocate for worst case
// const int MAX_ITERATIONS = 1000;  // Reserved for future use
...
private:
    ...
    bool is_3d;
    bool use_parallel;
    mutable std::mutex cache_mutex; // Mutex for protecting transition_cache
    
public:
    MonotoneAbstraction(const MonotoneDynamics& dynamics, bool use_3d, bool parallel = false) 
        : dyn(dynamics), safe_set_size(0), transition_cache_size(0), is_3d(use_3d), use_parallel(parallel) {
    ...
    void removeBasisIdx(const int* basis_idx) {
        ...
        for (int i = 0; i < safe_set_size; ++i) {
            ...
            if (match) {
                ...
                for (int k = i; k < safe_set_size - 1; ++k) {
                    for (int j = 0; j < state_dim; ++j) {
                        safe_set_basis[k * MAX_STATE_DIM + j] = safe_set_basis[(k + 1) * MAX_STATE_DIM + j];
                    }
                }
                safe_set_size--;
    ...
    void computeSafeSet() {
        ...
        while (true) {
            ...
            if (unsafe_count == 0) {
                ...
                std::cout << "Unsafe basis elements: " << unsafe_count << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== " << (is_3d ? "3D" : "2D") << " Safe Set Computation for Vehicle Following (OpenCL-Ready) ===" << std::endl;
