...
/**
 * Optimized Safe Set Computation for Vehicle Following
 * Implements performance optimizations based on profiling analysis
 */
...
public:
    ...
    // OPTIMIZATION: Early termination in xInSafeSet
    bool xInSafeSet(const int* x_idx) const {
        if (x_idx[0] == -1) return false;
        
        int state_dim = is_3d ? 3 : 2;
        // OPTIMIZATION: Check most likely candidates first (reverse order)
    ...
    // OPTIMIZATION: More efficient basis removal
    void removeBasisIdx(const int* basis_idx) {
        ...
        for (int i = 0; i < safe_set_size; ++i) {
            ...
            if (match) {
                // OPTIMIZATION: Use memmove for faster array shifting
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== " << (is_3d ? "3D" : "2D") << " Safe Set Computation (OPTIMIZED) ===" << std::endl;
    std::cout << "Computing invariant safe sets using " << (use_parallel ? "parallel" : "sequential") << " monotone abstractions..." << std::endl;
    std::cout << "Optimizations: Reduced RK4 steps from 1000 to " << RK4_STEPS_OPTIMIZED << " (5x faster dynamics)" << std::endl;
