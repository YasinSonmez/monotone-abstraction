function [con, veh_follow_abs] = get_3D_box_abs_v2()
% create 3D box abstraction for vehicle following scenario:
...
% create abstraction
veh_follow_abs = box_abstraction(veh_follow_dyn);
...
% state unboundedness
x_unbounded = [0 1; % h domain is unbounded
               0 0;
               0 0];
veh_follow_abs.set_x_unbounded(x_unbounded);

% input constraints
u_range = [con.T_brake_min con.T_max];
veh_follow_abs.set_u_range(u_range);

% input unboundedness
u_unbounded = zeros(size(u_range));
veh_follow_abs.set_u_unbounded(u_unbounded);

% disturbance constraints
w_range = [con.T_brake_max con.T_max];
veh_follow_abs.set_w_range(w_range);

% disturbance unboundedness
w_unbounded = zeros(size(w_range));
veh_follow_abs.set_w_unbounded(w_unbounded);
