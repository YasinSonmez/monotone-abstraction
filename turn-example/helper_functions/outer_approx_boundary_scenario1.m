function v = outer_approx_boundary_scenario1(s, s0)
...
% (s > con.collision_zone_width) before the oncoming vehicle enters the
...
% assume oncoming vehicle traveling at fastest speed (to be conservative)
t0 = (-con.collision_zone_width - s0) / con.v0_max;

if(t0 < 0)
    % no chance to cut in front => set v = v_max
    v = con.v_max;
elseif(s > con.collision_zone_width)
    % we already completed the turn => set v = v_min
    v = con.v_min;
else
    v = (con.collision_zone_width - s - 0.5*con.a_max_ego*(t0)^2) / t0;
